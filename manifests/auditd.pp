...
class os_hardening::auditd (
  ...
  if $enable {
    ...
    exec { 'CIS DIL Benchmark 4.1.3 - Ensure auditing for processes that start prior to auditd is enabled - disable':
      command => "/bin/sed -i -e 's/GRUB_CMDLINE_LINUX_DEFAULT=\"\\(.*\\)audit=1\\(.*\\)\"/GRUB_CMDLINE_LINUX_DEFAULT=\"\\1\\2\"/g' /etc/default/grub;",
      onlyif  => "/bin/grep GRUB_CMDLINE_LINUX /etc/default/grub | /bin/grep -q 'audit=1'",
      notify  => Exec['CIS DIL Benchmark 4.1.3 - Ensure auditing for processes that start prior to auditd is enabled - update grub'],
    }
  }
